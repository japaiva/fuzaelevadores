...
class PricingService:
    ...
    @classmethod
    def calcular_formacao_preco(cls, custo_total: float, tipo_faturamento: str = "Elevadores") -> Dict[str, Any]:
        """
        Calcula a formação de preço completa
        
        Args:
            custo_total (float): Custo total de produção
    ...
    @classmethod
    def recalcular_com_desconto(cls, formacao_preco: Dict[str, Any], preco_final_sem_impostos: float) -> Dict[str, Any]:
        """
...
class FormacaoPreco:
    ...
    def __init__(self, custo_producao: float, percentual_margem: float = 30.0, 
                 percentual_comissao: float = 3.0, percentual_impostos: float = 10.0, 
                 valor_desconto: float = 0.0):
        self.custo_producao = float(custo_producao)
        self.percentual_margem = float(percentual_margem)
        self.percentual_comissao = float(percentual_comissao)
        self.percentual_impostos = float(percentual_impostos)
    ...
    @property
    def preco_com_margem(self) -> float:
        return self.custo_producao + self.valor_margem

    @property
    def base_calculo_comissao(self) -> float:
    ...
    @property
    def percentual_desconto_real(self) -> float:
    ...
    def definir_preco_sem_impostos(self, preco: float):
        """Define um novo preço sem impostos solicitado"""
        self.preco_sem_impostos_solicitado = float(preco)

    def definir_desconto(self, valor_desconto: float):
